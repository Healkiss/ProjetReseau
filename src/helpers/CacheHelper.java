...
public class CacheHelper {
	...
	public static void insertContact(ContactServer monContact) {
		ObjectOutputStream oos;
		try{
	...
	public static Set<ContactServer> selectAllContacts() {
		ObjectInputStream ois = null;
		Set<ContactServer> myContacts= new HashSet<ContactServer>();
		...
		if (mFichier.exists()){
			try
			{
				...
				while(mBufferedIS.available() > 0 ){
					ois = new ObjectInputStream(mBufferedIS);
					ContactServer monContact = (ContactServer)ois.readObject();
